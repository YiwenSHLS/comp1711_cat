#include "utilities.h"
 ...
 int testbad(const char *line, char *date, float *bloodIron)
    {
        if(sscanf(line, "%[^,]%f", date, bloodIron) ==2){
            return 1;
        }
        else{
            return 0;
...
int main()
{
    ...
    while (1)
    {
        int test = testbad(line, daily_readings[counter].date, &daily_readings[counter].bloodIron);
        if (test == 0){
            printf("Error: Bad data: some data missing\n");
            return 1;
        }
